...
void DepthImageRenderer::initContext(GLContextData& contextData) const
{
    ...
    /* Create the Gray-Scott shader: */
    dataItem->depthShader = linkVertexAndFragmentShader("GrayScottShader");
    dataItem->depthShaderUniforms[0] = glGetUniformLocationARB(dataItem->depthShader, "tex");
    dataItem->depthShaderUniforms[1] = glGetUniformLocationARB(dataItem->depthShader, "time");
    dataItem->depthShaderUniforms[2] = glGetUniformLocationARB(dataItem->depthShader, "resolution");
    dataItem->depthShaderUniforms[3] = glGetUniformLocationARB(dataItem->depthShader, "feed");
    dataItem->depthShaderUniforms[4] = glGetUniformLocationARB(dataItem->depthShader, "kill");
}

	
GLuint DepthImageRenderer::linkVertexAndFragmentShader(const char* shaderName)
{
    GLuint vertexShader = glCreateShader(GL_VERTEX_SHADER);
    GLuint fragmentShader = glCreateShader(GL_FRAGMENT_SHADER);
    
    // Load shader source code
    std::string vertexShaderPath = std::string(shaderName) + ".vs";
    std::string fragmentShaderPath = std::string(shaderName) + ".fs";
...
std::string DepthImageRenderer::loadShaderSource(const std::string& filePath) const
{
...
void DepthImageRenderer::checkShaderCompileStatus(GLuint shader) const
{
    GLint status;
...
void DepthImageRenderer::checkProgramLinkStatus(GLuint program) const
{
    GLint status;
...
void DepthImageRenderer::renderDepth(const PTransform& projectionModelview, GLContextData& contextData) const
{
    ...
    /* Bind the Gray-Scott rendering shader: */
    ...
    /* Set other uniforms for the Gray-Scott shader: */
    glUniform1fARB(dataItem->depthShaderUniforms[1], /* time value */);
    glUniform2fARB(dataItem->depthShaderUniforms[2], /* resolution values */);
    glUniform1fARB(dataItem->depthShaderUniforms[3], /* feed value */);
    glUniform1fARB(dataItem->depthShaderUniforms[4], /* kill value */);
